...
use rama_syntax::{analyze, emit_clojure, emit_edn};
...
fn main() -> ExitCode {
    ...
    match command {
        ...
        "emit-edn" => match parse_path_and_output(&args[1..]) {
            Ok((path, output)) => emit_edn_file(&path, output.as_deref()),
            Err(message) => {
                eprintln!("{message}\n{}", usage());
                ExitCode::from(2)
            }
...
fn emit_edn_file(path: &Path, output: Option<&Path>) -> ExitCode {
    match emit_edn_source_file(path) {
        Ok(edn) => {
            if let Some(output) = output {
                if let Err(err) = write_file(output, &edn) {
                    eprintln!("failed to write {}: {err}", output.display());
                    return ExitCode::from(2);
                }
            } else {
...
fn emit_edn_source_file(path: &Path) -> Result<String, ()> {
    let src = fs::read_to_string(path).map_err(|err| {
        eprintln!("failed to read {}: {err}", path.display());
    })?;

    match analyze(&src) {
        Ok((file, result)) if result.ok() => Ok(emit_edn(&file)),
        Ok((_file, result)) => {
            for diagnostic in &result.diagnostics {
                eprintln!("{}", diagnostic.render(&src));
            }
