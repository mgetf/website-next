...
function getDismissedFromCookie(): Set<number> {
  ...
  try {
    const ids = match[1].split(',').map(Number).filter((n) => !isNaN(n));
    return new Set(ids);
  } catch {
    return new Set();
  }
	...
	<div class="w-full px-3 sm:px-4 py-2 sm:py-4 space-y-2">
		<div class="max-w-7xl mx-auto space-y-2">
			{#each visibleAnnouncements as announcement (announcement.id)}
				<div class="relative bg-gradient-to-r from-red-800/90 to-red-600/80 rounded-lg px-4 py-3 pr-10 border border-red-700/50">
					<p class="text-gray-100 text-center text-sm sm:text-base font-medium">{announcement.content}</p>
					<button
