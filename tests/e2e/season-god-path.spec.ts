...
test('seed league, create home team, seed away + paid teams, open sessions', async ({
  ...
  // Parse team ID from the redirect URL (/teams/:id) — no name index available in Rama.
  const teamUrl = homeCaptain.page.url();
  const teamUrlMatch = teamUrl.match(/\/teams\/(\d+)/);
  expect(teamUrlMatch).not.toBeNull();
  homeTeamId = parseInt(teamUrlMatch![1]!, 10);
  expect(homeTeamId).toBeGreaterThan(0);
  expect(await getTeamStatus(homeTeamId)).toBe('UNREADY');
});
