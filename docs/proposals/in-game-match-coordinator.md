...
`whitelistId` is `string | null`. `null` means this match does not set a whitelist of its own. The arena cfg `whitelist_id` still applies. A string (whitelist.tf numeric id or slug) is a locked override for that session via `MGE_SetArenaWhitelistOverride`. Per-client enforcement lives in the `mge-whitelist` plugin, not `mp_tournament_whitelist`. Coordinator v1 still does not have to send a non-null id.
...
| E36 | Whitelist / banned weapons                   | Payload `whitelistId` is `string \| null`. `null` leaves the arena cfg. A string calls `MGE_SetArenaWhitelistOverride` and stays locked for the session. Applied per client by `mge-whitelist`, not the tournament cvar. Coordinator v1 may still send `null`. |
...
| 2026-09-25 | Whitelist: `whitelistId` is `string \| null`. `null` does not clear an arena cfg list. A string is a locked per-session override. Enforcement is per client in `mge-whitelist`, not TFTrue/tf2rue's server-wide `mp_tournament_whitelist`. Native `MGE_SetArenaWhitelistOverride` ships in MGEMod. Coordinator v1 still does not need to send a non-null id. E36 updated.                                                |
