...
| **P-final** | [Gap 13: No Onboarding Docs](#gap-13-no-contributor-onboarding-documentation)              | High     | Medium | Done last — `CONTRIBUTING.md` should describe the _stable_ final codebase, not an intermediate state. Execute in Phase 9 immediately before opening the project. |
...
- **Tournament Unification** will rewrite `tournaments.ts`, `/tournaments` routes, and player profile tournament sections. Gap fixes applied to those files before the rewrite are wasted effort.
...
2. **Don't fully remediate gaps in code that a proposal will rewrite.** Specifically: don't apply Zod migration (Gap 1), error shape standardization (Gap 2), or service error decoupling (Gap 3) to `tournaments.ts` or its routes — those files are about to be replaced.
3. **CI and formatting before everything.** Every subsequent change benefits from automated checking.
4. **Onboarding docs are last, not first.** The codebase won't be open to contributors until the gaps are closed. `CONTRIBUTING.md` and contributor-facing docs should reflect the _final_ stable state of the codebase, not an intermediate one. Write them after the structural work is done.

### Recommended Execution Order

| Phase   | Work                                                                                                                                                                                                                                                     | Est. Effort | Rationale                                                                                                              |
| ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------- |
| **1** ✓ | **Gaps P0**: CI pipeline + Prettier                                                                                                                                                                                                                      | ~1 hour     | Done. Foundation for all subsequent automated checks.                                                                  |
| **2** ✓ | **Gaps P1 (code only)**: Client/server boundary fix (`ProfileMatch` → `$lib/types/match`), shared format constants (`$lib/constants/formats.ts`). `AGENTS.md` preamble updated for AI accuracy.                                                          | ~1 hour     | Done. Immediate code correctness fixes; contributor docs deferred until the project is ready to open.                  |
| **3**   | **UI Component Centralization Phase 1+2 only**: Design tokens in `app.css` + build Button/Card/Badge components. Do NOT migrate existing pages yet.                                                                                                      | ~2.5 hours  | Creates the component vocabulary. Existing pages keep working unchanged.                                               |
| **4**   | **Tournament Unification**: Schema migration, data migration, service rewrite, new UI built using the Phase 3 components from the start.                                                                                                                 | ~2-3 days   | The largest structural change. Benefits from CI (Phase 1) and components (Phase 3).                                    |
| **5**   | **UI Component Centralization Phase 3+4**: Migrate remaining existing pages to tokens + components. Skip tournament pages (already new). Delete FormDialog. Fix Toast/hex inconsistencies.                                                               | ~4-5 hours  | Finishes the UI consistency pass. Tournament pages excluded since they were built with components natively.            |
| **6**   | **Gaps P2**: Env var consolidation (call `validateEnvironment()`, migrate `process.env` callers), service error decoupling (replace `error()` → `notFound()`/`badRequest()` in 11 services), error shape standardization (decide Shape A vs B, migrate). | ~1-2 days   | Apply to the _stable_ codebase after structural rewrites are done. No wasted effort on files rewritten in Phase 4.     |
| **7**   | **Gaps P3**: Zod validation migration (17 files, 40+ actions), catch block typing (`err: unknown`), test infrastructure setup (Vitest + priority tests for permissions, payments, roster management).                                                    | ~3-5 days   | The longest remediation phase. Best done after all structural changes are complete so tests cover the final code.      |
| **8**   | **Gaps P4**: Audit logging gaps (add `logAudit` to 8 untracked actions), load function audit (verify all routes shape data properly).                                                                                                                    | Incremental | Do opportunistically when touching those files, or as a dedicated cleanup pass.                                        |
| **9**   | **Open source prep** (Gap 13): Finalize `CONTRIBUTING.md` to reflect the actual stable codebase. Delete this document — gaps are closed. Remove the preamble from `AGENTS.md`. Consider ADRs for major decisions.                                        | ~1-2 hours  | Done last. Contributor docs should describe the _final_ state. This phase marks the project as publicly contributable. |

### What Goes Obsolete

- **This entire document** is deleted in Phase 9 once gaps are closed. It is a temporary remediation tracker, not a permanent architectural reference.
- **This document's affected file counts** (Gaps 1, 2, 3) will shrink by a few files after tournament unification rewrites `tournaments.ts` and its routes. Update the tables after Phase 4.
...
- **Format System** and **Tournament Unification** proposals themselves will not go obsolete from any gap remediation.
...
| `form-action-patterns.mdc`        | Gaps 1, 2        | **Aspirational** — describes Zod + helpers pattern that only 3 files follow. Will cause friction until Gap 1 is remediated. Consider softening to "prefer" rather than "must" until migration is done. |
| `env-variable-access.mdc`         | Gap 4            | **Aspirational** — describes centralized access that 8 files violate.                                                                                                                                  |
| `client-server-type-boundary.mdc` | Gap 5, 12        | Matches intent. One known violation.                                                                                                                                                                   |
| `service-layer-conventions.mdc`   | Gaps 3, 10       | **Partially aspirational** — the error utility guidance conflicts with what 11 services actually do.                                                                                                   |
...
**Recommendation:** For aspirational rules, add a note at the top: _"Note: This rule describes the target state. Some existing code does not yet conform. When touching existing files, migrate toward this pattern."_ This prevents LLMs from being confused when they see code that contradicts the rule.
