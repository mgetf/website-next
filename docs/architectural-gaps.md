...
| **P1**   | [Gap 13: No Onboarding Docs](#gap-13-no-contributor-onboarding-documentation)              | High     | Medium | Without docs, contributors can't know the rules exist. Must precede opening the project.                    |
...
4. **Onboarding docs before opening to contributors.** Without docs, the rules are invisible outside Cursor.

### Recommended Execution Order

| Phase | Work                                                                                                                                                                                                                                                     | Est. Effort | Rationale                                                                                                                                  |
| ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| **1** | **Gaps P0**: CI pipeline + Prettier (run `bun run format` once, enable in CI)                                                                                                                                                                            | ~1 hour     | Foundation. Every subsequent change is protected by automated checks.                                                                      |
| **2** | **Gaps P1**: Onboarding docs (finalize `CLAUDE.md`, create `CONTRIBUTING.md`), client/server boundary fix (move `ProfileMatch` type), shared format constants (`$lib/constants/formats.ts`)                                                              | ~2 hours    | Makes the codebase legible to foreign eyes. Low-effort, high-prevention-value fixes.                                                       |
...
| **4** | **Tournament Unification**: Schema migration, data migration, service rewrite, new UI built using the Phase 3 components from the start.                                                                                                                 | ~2-3 days   | The largest structural change. Benefits from CI (Phase 1), docs (Phase 2), and components (Phase 3).                                       |
| **5** | **UI Component Centralization Phase 3+4**: Migrate remaining existing pages to tokens + components. Skip tournament pages (already new). Delete FormDialog. Fix Toast/hex inconsistencies.                                                               | ~4-5 hours  | Finishes the UI consistency pass. Tournament pages are excluded since they were built with components natively.                            |
| **6** | **Gaps P2**: Env var consolidation (call `validateEnvironment()`, migrate `process.env` callers), service error decoupling (replace `error()` → `notFound()`/`badRequest()` in 11 services), error shape standardization (decide Shape A vs B, migrate). | ~1-2 days   | Apply to the _stable_ codebase after structural rewrites are done. No wasted effort on files that were rewritten in Phase 4.               |
| **7** | **Gaps P3**: Zod validation migration (17 files, 40+ actions), catch block typing (`err: unknown`), test infrastructure setup (Vitest + priority tests for permissions, payments, roster management).                                                    | ~3-5 days   | The longest remediation phase. Best done after all structural changes are complete so tests cover the final code, not intermediate states. |
