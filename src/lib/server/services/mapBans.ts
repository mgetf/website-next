 ...
 * @param boSeries - Best of series (3 or 5)
...
export async function processBanPickAction(
	...
	if (actionType === 'pick') {
		const pickCount = matchMapBan.actions.filter((a) => a.actionType === MapBanActionType.PICK).length + 1; // +1 for current action
		await assignMapToGames(match.id, arenaId, pickCount, match.boGames || null);
	}

	// Update match map ban
	const isComplete = actionCount + 1 >= (match.boSeries === 3 ? 6 : 8);
