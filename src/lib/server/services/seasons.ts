...
export type LeagueGrid = {
  seasonNums: number[];
  rows: {
    regionId: number;
    regionName: string;
    /** Parallel to seasonNums: season id when that region has that season, else null */
    cells: (number | null)[];
  }[];
};

/**
 * Build a region × seasonNum grid for a format.
 * Columns are the union of season numbers that exist for the format;
 * cells are null when a region has no season for that column.
 * Only visible (non-hidden) regions with at least one season are included.
 */
export async function getLeagueGridByFormat(formatId: number): Promise<LeagueGrid> {
  const seasons = await prisma.season.findMany({
    where: {
      formatId,
      region: { hidden: 0 },
    ...
    orderBy: [{ regionId: 'asc' }, { seasonNum: 'asc' }],
  });

  if (seasons.length === 0) {
    return { seasonNums: [], rows: [] };
  }

  const seasonNums = [...new Set(seasons.map((s) => s.seasonNum))].sort((a, b) => a - b);

  const byRegion = new Map<
    number,
    { regionId: number; regionName: string; seasons: Map<number, number> }
  >();

  for (const season of seasons) {
    let row = byRegion.get(season.regionId);
    if (!row) {
      row = {
      ...
      byRegion.set(season.regionId, row);
    }
    row.seasons.set(season.seasonNum, season.id);
  }

  const rows = [...byRegion.values()].map((row) => ({
    regionId: row.regionId,
    regionName: row.regionName,
    cells: seasonNums.map((num) => row.seasons.get(num) ?? null),
  }));
