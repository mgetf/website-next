...
**Note:** All architectural guardrails are now enforced by `scripts/boundary-check.ts` (54 checks), runnable locally via `bun run boundary-check` and in CI. All companion cursor rules match current reality.
