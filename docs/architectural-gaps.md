...
| `form-action-patterns.mdc`        | Gaps 1, 2        | **Partially enforced** — Gap 2 (error shape) is resolved; `formError()`/`validationError()` now produce Shape A. Gap 1 (Zod migration) still aspirational — only ~6 files use `validateForm()`. |
...
**Note:** All architectural guardrails are now enforced by `scripts/boundary-check.ts` (52 checks), runnable locally via `bun run boundary-check` and in CI. The remaining aspirational rule is `form-action-patterns.mdc` for Gap 1 (Zod validation migration).
