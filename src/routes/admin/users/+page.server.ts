...
import { getUsers, countUsers, updateUser, banUser } from '$lib/server/services/users';
...
export const actions: Actions = {
	updateUser: async ({ request, locals }) => {
		...
		try {
			...
			return fail(400, { error: error instanceof Error ? error.message : 'Failed to update user' });
		}
	},
	...
	banUser: async ({ request, locals }) => {
		...
		try {
			...
			return { success: true, message: `User ${severity.toLowerCase()} successfully!` };
		} catch (error) {
			console.error('Error banning user:', error);
			return fail(400, { error: error instanceof Error ? error.message : 'Failed to ban user' });
		}
	}
};
