...
import { handleDiscordCallback, formatDiscordUsername, getDiscordAvatarUrl } from '$lib/server/auth/discord';
...
export const GET: RequestHandler = async ({ url, request }) => {
	...
	try {
		...
		const { user: discordUser, steamId } = await handleDiscordCallback(code, state, request);
