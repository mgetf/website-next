<script lang="ts">
  ...
  // Initialise state: nothing selected. For 1v1 teams the user is auto-included when
  // the team is checked (handled in checkoutTeams derivation), so no player UI is needed.
  $effect(() => {
    ...
    <div class="mb-8 text-center">
      ...
      <p class="text-text-muted">
        {data.participations.length === 1
          ? 'Pay your signup fee to activate your registration'
          : 'Pay your signup fees across all pending registrations'}
      </p>
    </div>

    <Card padding="none" class="overflow-hidden">
      <!-- Step 1: Participation Selection (only shown when multiple) -->
      ...
      {#if allTeamsReady}
        ...
        <div class="p-6 border-b border-border-default space-y-4">
          ...
          <label for="accept-refund" class="flex items-start gap-3 cursor-pointer">
            ...
            />
            <span class="text-sm text-text-label">
              I understand that all fees are non-refundable. By completing this payment, I commit to
              participating for the full duration of the season.
            </span>
          </label>
          ...
          {#if !allAgreementsChecked}
            ...
            <div class="bg-surface-input rounded-lg p-5 mb-4">
              {#each selectedParticipations as p (p.teamId)}
                ...
                {/if}
              {/each}
              <div class="border-t border-border-input pt-3 mt-3 flex justify-between items-center">
