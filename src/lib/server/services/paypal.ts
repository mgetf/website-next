 ...
 */
export function validatePayPalCredentials(): { valid: boolean; error?: string } {
 ...
 */
export async function getPayPalAccessToken(): Promise<{ token: string | null; error?: string }> {
	const config = getPayPalConfig();
...
export async function createPayPalOrder(params: {
	...
	if (config.isTestMode) {
		...
		return {
			...
			order: {
				...
				status: 'CREATED',
				links: [
					{ rel: 'self', href: `https://test.paypal.com/orders/${mockOrderId}` },
					{ rel: 'approve', href: `https://test.paypal.com/approve/${mockOrderId}` }
				]
			}
		};
	}
	...
	try {
		...
		if (!response.ok) {
			...
			const sanitizedError = paypalError.includes('authentication') || paypalError.includes('credential')
...
export async function capturePayPalOrder(
	orderID: string,
	testData?: { steamId: string; teamId: number; amount: number; currency: string }
