		...
		<div class="mb-6 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
			...
			<p class="text-yellow-300/80 text-sm">
				...
				{#if data.previousSeasonTeams.length === 1}
					<a href="/teams/{data.previousSeasonTeams[0].id}" class="font-semibold underline hover:text-yellow-200">{data.previousSeasonTeams[0].name}</a>,
				{:else}
					{#each data.previousSeasonTeams as team, i}
						<a href="/teams/{team.id}" class="font-semibold underline hover:text-yellow-200">{team.name}</a>{i < data.previousSeasonTeams.length - 1 ? ', ' : ''}
					{/each},
				{/if}
