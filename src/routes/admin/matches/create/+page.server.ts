...
export const actions: Actions = {
	...
	previewMatches: async ({ request, locals }) => {
		...
		const playoffRound = playoffRoundRaw && playoffRoundRaw !== '' ? parseInt(playoffRoundRaw) : null;

		try {
			const teams = await getEligibleTeams(regionId, divisionId, seasonId);

			if (teams.length === 0) {
				return { preview: { teams: [], matchups: [], weekLabel: null, existingCount: 0, isPlayoff }, success: true };
			}
			...
			if (isPlayoff && playoffRound) {
				...
				if (!playoff || !playoff.numRounds) {
					return fail(400, { error: 'No playoff configuration found for this season' });
				}
	...
	createMatchSet: async ({ request, locals }) => {
		...
		console.log('Create match set params:', { regionId, divisionId, seasonId, weekNo, boSeries, arenaId, matchDateTime, mapBanPoolId, isPlayoff, playoffRound, boGames });
		...
		if (isPlayoff) {
			if (!playoffRound) {
				return fail(400, { error: 'Playoff round is required for playoff matches' });
			}
			if (!mapBanPoolId) {
		...
		try {
			...
			if (isPlayoff) {
				...
				if (homeTeamIds.length === 0 || awayTeamIds.length === 0) {
					return fail(400, { error: 'Please select teams for all playoff matchups' });
				}

				if (homeTeamIds.length !== awayTeamIds.length) {
					return fail(400, { error: 'Home and away team selections must match' });
				}
