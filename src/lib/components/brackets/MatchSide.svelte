<script lang="ts">
  ...
  const isHighlighted = $derived(!isByeSide && hover.label !== null && hover.label === side.label);
        ...
        {#each players as player, i}
          {#if player.href}
            ...
            >
              ...
              {:else}
                <div
                  class="w-5 h-5 rounded-full bg-surface-input flex items-center justify-center text-text-muted text-[10px] font-medium"
                >
    ...
    {:else}
      ...
      {#if player.href}
        <a href={player.href} class="shrink-0">
          ...
          {:else}
            <div
              class="w-6 h-6 rounded-full bg-surface-input flex items-center justify-center text-text-muted text-xs font-medium"
            >
              {player.name.charAt(0).toUpperCase()}
            </div>
          {/if}
