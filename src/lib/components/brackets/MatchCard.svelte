...
>
	{#if match.href}
		<a href={match.href} class={containerClasses}>
			{#if showMetaRow}
				<div class="bracket-meta-row flex items-center gap-2 border-b border-border-default bg-surface-input/40">
					{#if match.bestOf !== undefined}
						<Badge color="zinc" size="sm">Bo{match.bestOf}</Badge>
					{/if}

					{#if match.label}
						<span class="bracket-meta-text text-text-muted font-medium truncate flex-1">{match.label}</span>
					{:else}
						<span class="flex-1"></span>
					{/if}

					{#if isLive}
						<span class="flex items-center gap-1 text-primary-400 bracket-meta-text font-medium shrink-0">
	...
	{#if showPopover && hasGames && match.games}
		<div class="game-popover">
			...
			{#each match.games as game}
				...
				<div class="game-row">
					<span
						class="flex-1 text-right tabular-nums {s1Wins ? 'text-success-400 font-semibold' : 'text-text-muted'}"
					>
						{game.side1Score}
					</span>
					<span class="game-num">G{game.gameNumber}</span>
					<span
						class="flex-1 tabular-nums {s2Wins ? 'text-success-400 font-semibold' : 'text-text-muted'}"
					>
						{game.side2Score}
					</span>
				</div>
