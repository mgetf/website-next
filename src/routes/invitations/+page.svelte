...
<div class="min-h-[calc(100vh-4rem)] px-4 py-12">
	<div class="max-w-4xl mx-auto">
		<div class="mb-8">
			...
			<p class="text-gray-400">View and manage your pending team invitations and join requests</p>
		</div>
		...
		{#if data.invitations.length === 0}
			<div class="bg-zinc-900 border border-zinc-800 rounded-lg p-12 text-center">
				...
				<p class="text-gray-400 text-lg">You don't have any pending team invitations or join requests.</p>
			</div>
		{:else}
			<div class="space-y-4">
				{#each data.invitations as invitation}
					<div class="bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden">
						<div class="p-6 flex items-center justify-between gap-4">
							...
							<div class="flex items-center gap-4 flex-1 min-w-0">
								{#if invitation.team.avatar}
									...
									/>
								{:else}
									<div class="w-16 h-16 rounded-lg bg-zinc-800 border border-zinc-700 flex items-center justify-center flex-shrink-0">
										<span class="text-2xl text-gray-400">{invitation.team.name.charAt(0)}</span>
									</div>
								{/if}
								...
								<div class="flex-1 min-w-0">
									...
									<div class="flex flex-wrap gap-2 text-sm text-gray-400">
										{#if invitation.team.division}
											<span class="px-2 py-1 bg-zinc-800 rounded">{invitation.team.division.name}</span>
										{/if}
										{#if invitation.team.region}
											<span class="px-2 py-1 bg-zinc-800 rounded">{invitation.team.region.name}</span>
										{/if}
										{#if invitation.team.season}
											<span class="px-2 py-1 bg-zinc-800 rounded">Season {invitation.team.season.seasonNum}</span>
										{/if}
										<span class="px-2 py-1 bg-zinc-800 rounded">{invitation.team.players.length}/3 Players</span>
									</div>
								</div>
							</div>
							...
							<div class="flex items-center gap-2 flex-shrink-0">
								{#if invitation.status === 0}
									...
									<form
										method="POST"
										action="?/accept"
										use:enhance={enhanceWithLoading}
									>
										<input type="hidden" name="teamId" value={invitation.teamId} />
										<button
											type="submit"
											disabled={isSubmitting || data.rosterLocked}
											class="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 disabled:bg-emerald-600/50 disabled:cursor-not-allowed text-white rounded-lg font-medium transition-colors text-sm"
										>
									...
									>
										...
										<button
											...
											class="px-4 py-2 bg-zinc-800 hover:bg-zinc-700 disabled:opacity-50 disabled:cursor-not-allowed text-gray-300 rounded-lg font-medium transition-colors text-sm"
										>
											Decline
										</button>
									</form>
								{:else}
									...
									>
										...
										<button
											type="submit"
											disabled={isSubmitting}
											class="px-3 py-1.5 text-sm bg-zinc-800 hover:bg-zinc-700 disabled:opacity-50 disabled:cursor-not-allowed text-gray-400 rounded-lg transition-colors"
										>
											Withdraw
										</button>
									</form>
						...
						<div class="px-6 pb-4 border-t border-zinc-800/50 pt-3">
							<a
