 ...
 * Business logic validation:
 * - Region must exist
 * - Region must not have any seasons or teams
 */
export async function deleteRegion(id: number) {
	...
	// Check if region has seasons or teams
	if (region._count.seasons > 0 || region._count.teams > 0) {
		throw new Error(
			`Cannot delete region with ${region._count.seasons} seasons and ${region._count.teams} teams.`
		);
	}
