...
export const POST: RequestHandler = async ({ request, locals }) => {
	try {
		...
		if (!seasonId || typeof seasonId !== 'number') {
			return json({ error: 'Season ID is required and must be a number' }, { status: 400 });
		}
		...
		if (!isTournament) {
			if (!numRounds || typeof numRounds !== 'number' || numRounds < 1) {
				return json({ error: 'Number of rounds is required for non-tournament mode and must be >= 1' }, { status: 400 });
			}
		}
		...
		return json({ error: 'Failed to create playoff configuration' }, { status: 500 });
	}
};
